...
public:
    ...
    static void readCSV(const string& filename, vector<int>& x_values, vector<int>& y_values, vector<int>& type_values) {
        ...
        while (getline(file, line)) { // 逐行读取文件内容
            ...
            if (tokens.size() >= 5) { // 确保每行至少有3个字段（x，y，type）
                int x = stoi(tokens[2]);
                int y = stoi(tokens[3]);
                ...
                type_values.push_back(type); // 将y值添加到y_values数组
            }
        }
    ...
    //绘制散点图
    static void drawScatterPlot(const std::vector<int>& xData, const std::vector<int>& yData, const std::vector<int>& typeData)
    {
        // 创建一个具有边距的图像
        ...
        {
            int x = xData[i] + xOffset;
            int y = image.rows - (yData[i] + yOffset);
            int type = typeData[i];

            cv::Scalar color = colorMap[type];
    ...
    static void drawHistogramFliter(const vector<int>& histogram, const string& label) {
        const int histSize = histogram.size();  // 直方图的条目数量
        const int rangeMin = 0;   // 直方图范围最小值
        const int rangeMax = histogram.size();  // 直方图范围最大值
    ...
    static vector<double> rotatePointsX(const vector<int>& x_values, const vector<int>& y_values, int n) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // 计算旋转后的位置
            x_rotated_values.push_back((x * cos(angle) - y * sin(angle)));
    ...
    // 输入 x、y坐标数组以及其他参数，直方图从out_hist传出。
    // is_x = true 时计算x方向直方图，否则计算y方向直方图
    static int CalcHist2(const vector<double>& x, const vector<double>& y, vector<int>& out_hist, int histSize, bool is_x) {
        // 建立CalcHist函数内直方图统计结果存储向量
        ...
        //int histSize = 10;// ceil(rangeMax - rangeMin);  // 直方图的条目数量
        ...
        for (double value : values) { // 遍历数据数组
            if (value >= rangeMin && value <= rangeMax) { // 确保数据值在范围内
                int bin = round((value - rangeMin) / binWidth); // 计算数据值所属的条目索引 static_cast<int>
                out_hist[bin]++; // 相应条目计数加1
            }
        }

        // 创建直方图图像
        int histWidth = 600; // 直方图图像宽度
        int histHeight = 300; // 直方图图像高度
        double binWidthPx = (static_cast<double>(histWidth) / histSize); // 计算每个条目在图像中的宽度
        Mat histImage(histHeight, histWidth, CV_8UC3, Scalar(0, 0, 0)); // 创建黑色图像
